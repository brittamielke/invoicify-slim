...
public class InvoicesController {
	...
	public ModelAndView showInvoices() {
		...
		mv.addObject("showTable", invoiceRepo.count() > 0);
		return mv;
	}
	...
	public ModelAndView createInvoice(@PathVariable Long clientId) {
		...
		List<BillingRecord> records = billingRecordRepo.findByClientIdAndLineItemIsNull(clientId);
	...
	public String createInvoice(Invoice invoice, @PathVariable Long clientId, long[] recordIds, Authentication auth) {
		...
		ArrayList<BillingRecord> records = new ArrayList<BillingRecord>();
		for (long id : recordIds) {
			records.add(billingRecordRepo.findById(id));
		}

		// Create a new list that can hold invoice line items
		ArrayList<InvoiceLineItem> items = new ArrayList<InvoiceLineItem>();
